...
As a [Windows user](http://conan.is/clojure/clojurescript/windows/2015/10/05/clojure-on-windows.html), I'm used to touching my laptop screen, and most of your users today are probably used to fingering, rather than clicking, the internet on their phones or tablets.  [Hammer.js](http://hammerjs.github.io/) is a lovely JavaScript library for working with touch gestures, and ClojureScript's interop makes it easy to use. Let's get touchy-feely.
 
 
# Introduction

Hammer uses things called Recognisers to listen for touch gestures.  We'll be using three of them: [pinch](http://hammerjs.github.io/recognizer-pinch/), [pan](http://hammerjs.github.io/recognizer-pan/) and [tap](http://hammerjs.github.io/recognizer-tap/).  We'll need to make and configure an instance of [Hammer.Manager](http://hammerjs.github.io/api/#hammer.manager), and the same for each of the three Recognisers; then we'll plug them together, and finally we'll add some handler functions to do the actual work.
...
We don't need much for this, just Hammer and Reagent:
...
Let's start by creating a new Reagent component, which contains an image. Later we'll add the ability to zoom and pan that image:
...
Now we've got a component that displays an image, it's Hammer time.  The `Hammer.Manager` needs to bind to a DOM node, so we have to create it after React has mounted our component.  We can add a `:component-did-mount` lifecycle handler, which gets a reference to our component's node passed in by Reagent.  

_You'll notice we've used the [`js-invoke`](http://cljs.github.io/api/cljs.core/#js-invoke) function to call the manager's functions - the stubs in the `[cljsjs/hammer "2.0.4-5"]` jar don't work, so we call the manager directly._
...
You can find the source on [GitHub](https://github.com/conan/zoomer/blob/master/src/cljs/zoomer/core.cljs).  The final `zoomer.core` namespace looks like this:
